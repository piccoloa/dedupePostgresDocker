...
if os.path.exists(settings_file):
    ...
    if os.path.exists(training_file):
        logger.info('reading labeled examples from ', training_file)
    ...
    deduper.train(recall=0.60)
    ##alex (recall=0.90, True)
...
logger.info('writing blocking map')
...
logger.info('prepare blocking table. this will probably take a while ...')

logger.info("indexing block_key")
...
logger.info("adding donor_id index and sorting index")
...
def candidates_gen(result_set):
    ...
    for row in result_set:
        if row['block_id'] != block_id:
            ...
            if i % 10000 == 0:
                logger.info(i, "blocks")
                # logger.info(time.time() - start_time, "seconds")
...
logger.info('creating entity_map database')
...
logger.info('# duplicate sets')
logger.info(len(clustered_dupes))
...
logger.info("Top Donors (deduped)")
for row in c.fetchall():
    #row['totals'] = locale.currency(row['totals'], grouping=True)
    logger.info('%(totals)20s: %(name)s' % row)
...
logger.info('ran in %0.2f seconds' % (time.time() - start_time))
