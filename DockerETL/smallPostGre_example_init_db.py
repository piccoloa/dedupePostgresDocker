...
_file = 'small_Illinois-campaign-contributions'
...
if not os.path.exists(contributions_csv_file):
    ...
    with open(contributions_csv_file, 'w') as csv_file:
...
try:
    ...
    # necessary to issue rollback, otherwise next instructions will be ignored
    c.execute("ROLLBACK")
    conn.commit()
    pass
